// ⚠️ СТОРОЖ АВТОРА ЧЕКА: показ и отбор на ОДНОМ поле (04.09.2026).
//
// ЗАЧЕМ, ЗАМЕРОМ. На проде в 88 чеках колонка `employee` пуста во всех,
// а `user_id` заполнен во всех. Фронт подставлял вместо пустоты жёстко
// вписанное имя владельца (`r.employee || "Алексей Шукалович"`), и это давало
// два следствия: на «Сводке» ВСЕ расходы организации складывались в одного
// человека, а фильтр «Сотрудник» по любому другому имени давал пустой список
// ВСЕГДА — сравнивалась подставленная строка, а не автор.
//
// ⚠️ ЖИВЫЕ ДАННЫЕ ЭТОГО НЕ ПОКАЖУТ: у всех чеков прода один автор, и экран
// после правки выглядит почти так же. Поэтому проверка — здесь, на данных
// ДВУХ авторов и одного чека без автора, а не «посмотрел глазами».
//
// ЗАПУСК: npm run author (нужен Chrome; без него — «ПРОВЕРКА НЕ ВЫПОЛНЕНА»).
import { execFileSync, spawn } from "node:child_process";
import { existsSync, readFileSync } from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const КОРЕНЬ = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const ПРОБА = path.join(КОРЕНЬ, "scripts/probe-avtor");
const ПОРТ = 5700 + Math.floor(Math.random() * 200);

const БРАУЗЕРЫ = [
  "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome",
  "/Applications/Chromium.app/Contents/MacOS/Chromium",
  "/usr/bin/google-chrome",
  "/usr/bin/chromium",
];

// Ожидания по ролям. Ключевое утверждение — второе: сотрудник подписей
// не видит, потому что список у него и так свой (решение владельца).
// Прогоны: два по чекам (роль решает, видна ли подпись) и один по «Сводке»,
// где живёт фильтр «Сотрудник» и разрез сумм по авторам.
const ПРОГОНЫ = [
  { имя: "чеки · admin", хвост: "?rol=admin", шаги: "cheki_admin" },
  { имя: "чеки · employee", хвост: "?rol=employee", шаги: "cheki_employee" },
  // ⚠️ ДВА ПРОГОНА ПО ЭКРАНУ «ЧЕКИ» — тому, который открывает человек.
  // Их не было, и поэтому отсутствие пункта «Сотрудник» в шторке «Чеков»
  // дожило до замера владельца глазами при зелёном сторожа.
  {
    имя: "фильтр·чеки · admin",
    хвост: "?rol=admin&rezhim=filtr",
    шаги: "filtr_admin",
  },
  {
    имя: "фильтр·чеки · employee",
    хвост: "?rol=employee&rezhim=filtr",
    шаги: "filtr_employee",
  },
  {
    имя: "сводка · admin",
    хвост: "?rol=admin&rezhim=svodka",
    шаги: "svodka",
  },
];

const ОЖИДАЕМО = {
  cheki_admin: [
    ["① экран чеков", "admin · Кофейня+Канцтовары+Такси"],
    ["② подпись автора в списке", "Шукалович А.+Иванова Т."],
    ["②б подпись в списке — своей строкой", "своей строкой, ниже колонок"],
    ["③ автор в карточке чужого чека", "Иванова Т."],
    ["③б автор стоит в шапке, рядом с датой", "под датой, одна колонка"],
    ["④ закрыта карточка", "список"],
    ["⑤ автор у чека без user_id", "Автор не указан"],
  ],
  cheki_employee: [
    ["① экран чеков", "employee · Кофейня+Канцтовары+Такси"],
    ["② подпись автора в списке", "нет подписей"],
    ["②б подпись в списке — своей строкой", "подписей нет"],
    // ⚠️ ОТМЕНА РЕШЕНИЯ 08.09.2026 «АВТОР ВИДЕН ВСЕМ В КАРТОЧКЕ».
    // Прежний довод: сотрудник чужую карточку открыть не может (404),
    // значит вреда нет. Новый довод владельца: значит и ПОЛЬЗЫ нет —
    // подпись у него всегда одна и та же и ничего не различает.
    // Карточка была ЕДИНСТВЕННЫМ местом без проверки роли: список чеков
    // и список отчётов спрашивают canApprove, она — нет.
    ["③ автор в карточке чужого чека", "НЕ ВИДЕН"],
    ["③б автор стоит в шапке, рядом с датой", "ПОДПИСИ В ШАПКЕ НЕТ"],
    ["④ закрыта карточка", "список"],
    // ⚠️ «Автор не указан» сотрудник тоже НЕ ВИДИТ, и это следствие того же
    // решения, а не отдельное. Заглушка нужна ролям, которые сравнивают
    // чеки разных людей: им пустое место means «чей — неизвестно». У
    // сотрудника сравнивать не с чем, и заглушка была бы шумом с текстом.
    ["⑤ автор у чека без user_id", "НЕТ"],
  ],
  filtr_admin: [
    ["① экран чеков", "admin · Кофейня+Канцтовары+Такси"],
    ["② открыт фильтр", "фильтр открыт"],
    // ⚠️ ГЛАВНОЕ: дотянуться до отбора с экрана вообще можно.
    ["③ пункт «Сотрудник» в шторке", "пункт есть"],
    [
      "③в в фильтре имя ПОЛНОЕ, а не подпись",
      "полные: Алексей Шукалович+Пустой Человек+Татьяна Иванова",
    ],
    ["④а отмечен сотрудник БЕЗ чеков", "отмечен"],
    ["④б применён фильтр", "пусто"],
    ["⑤а переоткрыт фильтр", "фильтр открыт"],
    ["⑤б отмечен сотрудник С чеками", "отмечен"],
    // Ровно его чек, а не «хоть что-то».
    ["⑤в применён фильтр", "Канцтовары"],
  ],
  filtr_employee: [
    ["① экран чеков", "employee · Кофейня+Канцтовары+Такси"],
    ["② открыт фильтр", "фильтр открыт"],
    // Своих чеков у сотрудника и так одни: выбор коллеги дал бы пустой экран.
    ["③ пункт «Сотрудник» в шторке", "пункта нет"],
  ],
  svodka: [
    ["① экран сводки", "admin · открыта"],
    // Разрез строится по автору, поэтому в нём оба человека и ничейный чек.
    ["② разрез по авторам", "Шукалович А.+Иванова Т.+Автор не указан"],
    ["③ открыт фильтр", "фильтр открыт"],
    // ⚠️ ГЛАВНОЕ УТВЕРЖДЕНИЕ ПРАВКИ, обе половины.
    ["④а отмечен сотрудник БЕЗ чеков", "отмечен"],
    ["④б применён фильтр", "пусто"],
    ["⑤а переоткрыт фильтр", "фильтр открыт"],
    ["⑤б отмечен сотрудник С чеками", "отмечен"],
    // ⚠️ РОВНО ЕГО ЧЕК, а не «хоть что-то»: у Ивановой один чек — «Канцтовары».
    ["⑤в применён фильтр", "Канцтовары"],
  ],
};

const беды = [];

// ── СТАТИЧЕСКАЯ ПОЛОВИНА, И ОНА ЗДЕСЬ ОБЯЗАТЕЛЬНА. Возврат локальной копии
// формата (`empName` в App.jsx) поведения НЕ МЕНЯЕТ — она была байт-в-байт
// равна `полноеИмя`. На экране такую мутацию поймать нечем: копия и общая
// функция дают одну строку. Ловится только чтением кода — тем же приёмом,
// что в стороже суммы (T177), и по той же причине.
const ЧИТАТЬ = (о) => readFileSync(path.join(КОРЕНЬ, о), "utf8");
const ПРИЛОЖЕНИЕ = ЧИТАТЬ("src/App.jsx");
console.log("  СТАТИЧЕСКАЯ ПОЛОВИНА");
{
  // Сборка имени вручную ВНУТРИ фильтра: `${first} ${last}` рядом с employees.
  // ⚠️ ПРОВЕРЯЕМ САМО МЕСТО, А НЕ ОТСУТСТВИЕ СЛОВА. Первая редакция искала
  // «const empName» и обратную косую перед шаблонной строкой — и пропустила
  // мутацию, которая собрала имя ТУТ ЖЕ, но иначе отформатированную. Признак
  // должен утверждать, ЧТО СТОИТ в чипе, а не чего в файле нет.
  const чип =
    /employees\.map\(\(u\) => \[полноеИмя\(u\), полноеИмя\(u\)\]\)/.test(
      ПРИЛОЖЕНИЕ,
    );
  if (!чип)
    беды.push("чип фильтра рисуется НЕ общей функцией — вернулась своя сборка");
  console.log(
    `  ${
      чип ? "✓" : "✗"
    } чип фильтра рисуется общей функцией полноеИмя (формат один)`,
  );
  const своя = /const empName\s*=/.test(ПРИЛОЖЕНИЕ);
  if (своя) беды.push("в App.jsx вернулась копия empName");
  console.log(`  ${своя ? "✗" : "✓"} копии empName в App.jsx нет`);
}
console.log("  ЭКРАННАЯ ПОЛОВИНА");

console.log("\nАВТОР ЧЕКА: показ и отбор на одном поле");

const браузер = БРАУЗЕРЫ.find((п) => existsSync(п));
if (!браузер) {
  console.log("  ✗ ПРОВЕРКА НЕ ВЫПОЛНЕНА: не найден Chrome — смотреть нечем");
  БРАУЗЕРЫ.forEach((п) => console.log(`      ${п}`));
  process.exit(1);
}

try {
  execFileSync(
    path.join(КОРЕНЬ, "node_modules/.bin/vite"),
    ["build", "--config", path.join(ПРОБА, "vite.config.mjs")],
    { cwd: КОРЕНЬ, stdio: "pipe", timeout: 180000 },
  );
} catch (е) {
  console.log("  ✗ ПРОВЕРКА НЕ ВЫПОЛНЕНА: проба не собралась");
  String(е.stdout || е.message)
    .split("\n")
    .slice(-6)
    .forEach((с) => console.log("      " + с));
  process.exit(1);
}

const сервер = spawn(
  path.join(КОРЕНЬ, "node_modules/.bin/vite"),
  [
    "preview",
    "--config",
    path.join(ПРОБА, "vite.config.mjs"),
    "--port",
    String(ПОРТ),
    "--strictPort",
  ],
  { cwd: КОРЕНЬ, stdio: "ignore" },
);

const снятьРазом = (хвост) =>
  new Promise((готово, споткнулись) => {
    const дитя = spawn(
      браузер,
      [
        "--headless",
        "--disable-gpu",
        "--virtual-time-budget=25000",
        "--window-size=430,1400",
        "--dump-dom",
        `http://localhost:${ПОРТ}/${хвост}`,
      ],
      { stdio: ["ignore", "pipe", "ignore"] },
    );
    let вывод = "";
    const часы = setTimeout(() => {
      дитя.kill("SIGKILL");
      споткнулись(new Error("браузер не ответил за 120 с"));
    }, 120000);
    дитя.stdout.on("data", (к) => (вывод += к));
    дитя.on("error", (е) => {
      clearTimeout(часы);
      споткнулись(е);
    });
    дитя.on("close", () => {
      clearTimeout(часы);
      готово(вывод);
    });
  });

const разобрать = (dom) => {
  const м = dom.match(/<div id="ЗАМЕР">([\s\S]*?)<\/div>/);
  if (!м || !м[1].trim()) return null;
  try {
    return JSON.parse(
      м[1]
        .replace(/&quot;/g, '"')
        .replace(/&amp;/g, "&")
        .replace(/&lt;/g, "<")
        .replace(/&gt;/g, ">"),
    );
  } catch {
    return null;
  }
};

try {
  let поднялся = false;
  for (let i = 0; i < 60 && !поднялся; i++) {
    await new Promise((г) => setTimeout(г, 250));
    try {
      поднялся = (await fetch(`http://localhost:${ПОРТ}/`)).ok;
    } catch {
      /* ещё не поднялся */
    }
  }
  if (!поднялся) throw new Error("сервер пробы не поднялся за 15 с");

  for (const прогон of ПРОГОНЫ) {
    console.log(`\n  ${прогон.имя.toUpperCase()}`);
    let замер = null;
    // ⚠️ Переснимаем только НЕСОСТОЯВШЕЕСЯ измерение (пусто, приложение
    // не встало). Расхождение шага — это результат, его не переснимают.
    for (let п = 0; п < 3 && !замер; п++) {
      const р = разобрать(await снятьРазом(прогон.хвост));
      if (Array.isArray(р)) замер = р;
    }
    if (!замер) {
      console.log("  ✗ ПРОВЕРКА НЕ ВЫПОЛНЕНА: замер не снялся за 3 попытки");
      process.exitCode = 1;
      continue;
    }
    ОЖИДАЕМО[прогон.шаги].forEach(([имя, ждём], i) => {
      const было = String(замер[i] ?? "")
        .split(": ")
        .slice(1)
        .join(": ");
      const ок = было === ждём;
      console.log(
        `  ${ок ? "✓" : "✗"} ${имя}: ${было || "НЕТ"}${
          ок ? "" : `  ← ждали «${ждём}»`
        }`,
      );
      if (!ок)
        беды.push(`${прогон.имя}/${имя}: «${было || "НЕТ"}» вместо «${ждём}»`);
    });
  }
} catch (е) {
  console.log(`  ✗ ПРОВЕРКА НЕ ВЫПОЛНЕНА: ${String(е.message).slice(0, 200)}`);
  сервер.kill("SIGKILL");
  process.exit(1);
} finally {
  сервер.kill("SIGKILL");
}

if (беды.length) {
  console.log(`\n  ⚠️ РАСХОЖДЕНИЙ ${беды.length}`);
  беды.forEach((б) => console.log(`     · ${б}`));
  process.exit(1);
}
console.log(
  "\n  ✓ автор из user_id: подписи по роли, «Автор не указан» на месте,\n" +
    "    фильтр «Сотрудник» на «Чеках» и «Сводке» отбирает по автору\n",
);
process.exit(0);
