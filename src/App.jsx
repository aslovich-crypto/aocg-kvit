...
// намеренно («в Приме её никто не понимает», `app/auth.py:102` — ссылка на
// :48 протухла, поправлена 08.09.2026), прав ей не давал ни один гейт, а
// строк с такой ролью на проде нет: после чистки 31.08.2026 в базе один
// человек, администратор. Подпись без поведения — это обещание, которого
// никто не выполнит.
//
// ⚠️ ЭТОТ СПИСОК СВЕРЯЕТСЯ С БЭКЕНДОМ, А НЕ ДЕРЖИТСЯ НА ПАМЯТИ (T39, заход 3).
// Набор идентификаторов порождён из `ROLES` в app/auth.py (S-24) и лежит в
// src/lib/roles.js; scripts/check-roles.mjs сверяет, что здесь ровно он.
// Подписи и описания остаются решением экрана — словарь СВЕРЯЕТ, не копирует.
const ROLES = [
  {
    id: "employee",
    label: "Сотрудник",
...
function RegisterScreen({ onAuthed, navigate }) {
  ...
  async function submit() {
    ...
    // 08.09.2026). Приглашающий часто знает человека по имени и вписал бы
    // что попало — в авансовом отчёте, по которому возмещают деньги, оказался
    // бы мусор вместо пустоты. Пустое поле честнее неверного, а свою фамилию
    // человек знает. Поле тут было нарисовано и раньше, просто не требовалось.
    // ⚠️ Форму СОЗДАНИЯ приглашения (T168) это не трогает: её только что
    // упростили, и лишнее обязательное поле там было бы шагом назад.
    if (!f.last_name.trim()) {
      setErr("Укажите фамилию");
      return;
    }
