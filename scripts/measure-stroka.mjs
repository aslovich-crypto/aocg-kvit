  ...
  for (const р of с.строки) {
    ...
    console.log(
      `  #${р["№"]} строка ${р.строка.w}×${р.строка.h} подложка=${
        ...
        `\n      мета cw=${р.мета.cw} sw=${р.мета.sw}` +
        ` ${р.мета.обрезан ? "ПЕРЕПОЛНЕНА" : "влезает"}` +
...
try {
  ...
  const дом = await снять();
  const з = разобрать(дом);
  if (!з) {
    console.log("  ✗ ЗАМЕР НЕ ВЫПОЛНЕН: проба не отдала чисел");
    process.exit(1);
  }
  if (з.НЕ_ОТРИСОВАЛОСЬ) {
    console.log("  ✗ ЗАМЕР НЕ ВЫПОЛНЕН: экран не отрисовался");
    process.exit(1);
  }

  печать(з.до);
  печать(з.после);
  if (з.потолок) печать(з.потолок);

  const д = з.до,
    п = з.после;
  const обрезанныхДо = д.строки.filter(
    (р) => р.автор && р.автор.обрезан,
  ).length;
  const обрезанныхПосле = п.строки.filter(
    (р) => р.автор && р.автор.обрезан,
  ).length;
  console.log("\n═══ ИТОГ ═══");
  console.log(
    `  авторов обрезано: было ${обрезанныхДо} из ${д.строки.length}` +
      ` · стало ${обрезанныхПосле} из ${п.строки.length}`,
  );
  console.log(
    `  высота строки: было ${д.строки[0]?.строка.h} · стало ${п.строки[0]?.строка.h}`,
  );
  console.log(
    `  видно целиком: было ${д.видноЦеликом} · стало ${п.видноЦеликом}`,
  );
  if (з.потолок) {
    const пт = з.потолок;
    const обр = пт.строки.filter((р) => р.автор && р.автор.обрезан).length;
    console.log(
      `  ПОТОЛОК (вниз и карта, и дата): обрезано ${обр} из ${пт.строки.length}` +
        ` · высота ${пт.строки[0]?.строка.h} · видно ${пт.видноЦеликом}`,
    );
  }
  console.log(`\n═══ ЖЕСТ ═══`);
  const уехала = (ж) => /translateX\(-\d/.test((ж && ж.вовремя) || "");
  for (const [метка, ж] of [
    ["однострочная (контроль)", д.жест],
    ["двойная (гипотеза)", п.жест],
  ]) {
    if (!ж) {
  ...
  if (!уехала(д.жест)) {
    ...
    console.log("     не доходит. Про свайп при двойной строке этот замер");
    console.log("     НЕ ОТВЕЧАЕТ; нужен другой способ, а не вывод по нему.");
  }
} finally {
  сервер.kill("SIGKILL");
}
